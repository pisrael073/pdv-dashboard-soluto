...
def _gc():
    ...
    if CREDS_PATH.exists():
        credentials = ServiceAccountCredentials.from_json_keyfile_name(str(CREDS_PATH), scopes=scope)
        return gspread.authorize(credentials)
...
def pagina_presupuesto_cumplimiento():
    ...
    * {
    ...
    # ── SECCIÓN DE ANÁLISIS DE DN ─────────────────────────────────────────────
    st.markdown('<h2 class="section-title">👥 ANÁLISIS DE DN</h2>', unsafe_allow_html=True)

    # Gauge de cumplimiento DN global
    fig_dn_gauge = go.Figure(data=[go.Indicator(
        mode="gauge+number+delta",
        value=cump(tdn, mdn),
        title={'text': "Cumplimiento DN Global"},
    ...
    fig_dn_gauge.update_layout(height=400, paper_bgcolor='#0d1117',
                               font=dict(color='#cbd5e1'), showlegend=False)
    st.plotly_chart(fig_dn_gauge, use_container_width=True)

    # Ranking de vendedores por DN
    top_dn = tabla.nlargest(10, 'DN_REAL')[['NOMBRE', 'DN_REAL', 'META_DN', 'CUMP_DN']].reset_index(drop=True)
    fig_top_dn = go.Figure()
    fig_top_dn.add_trace(go.Bar(
        y=top_dn['NOMBRE'],
        x=top_dn['DN_REAL'],
        orientation='h',
        name='DN Real',
        marker=dict(color='#3b82f6'),
        text=[f"{v:.0f}" for v in top_dn['DN_REAL']],
        textposition='auto',
    ))
    fig_top_dn.add_trace(go.Scatter(
        y=top_dn['NOMBRE'],
        x=top_dn['META_DN'],
        mode='markers+text',
        name='Meta DN',
        marker=dict(symbol='diamond', size=12, color='#ef4444'),
        text=[f"Meta: {v:.0f}" for v in top_dn['META_DN']],
        textposition='top center',
    ))
    fig_top_dn.update_layout(
        title="🏆 Top 10 Vendedores por DN",
    ...
    st.plotly_chart(fig_top_dn, use_container_width=True)

    # Treemap de DN por zona
    dn_zona = tabla.groupby('ZONA')[['DN_REAL']].sum().reset_index()
    fig_dn_treemap = go.Figure(go.Treemap(
        labels=dn_zona['ZONA'],
        parents=[''] * len(dn_zona),
        values=dn_zona['DN_REAL'],
        marker=dict(colors=['#FF6B35', '#1E88E5'],
                   line=dict(color='#0d1117', width=2)),
        text=[f"<b>{z}</b><br>{int(v)} clientes" for z, v in zip(dn_zona['ZONA'], dn_zona['DN_REAL'])],
        textposition='middle center',
        textfont=dict(size=16, color='white'),
        hovertemplate='<b>%{label}</b><br>DN: %{value}<extra></extra>',
    ))
    fig_dn_treemap.update_layout(
        title="📊 DN por Zona",
        height=300,
        paper_bgcolor='#0d1117',
        font=dict(color='#cbd5e1'),
    ...
    st.plotly_chart(fig_dn_treemap, use_container_width=True)
